...
\centering
\begin{tikzpicture}
	\draw(0,0) circle[radius=1.9cm];
	\fill[blue] circle[radius=0.5mm]node[below,rotate=30,black]{\small diámetro};
	\draw[<->, thick, blue,rotate=30] (-1.9,0) to (1.9,0) node[pos=.6,above, black] {\small d};
	\node at (0,0){\tiny $\times$};
	\draw[<->, thick, blue, rotate=80](1.98,.71)arc(20:360:2.1);
	\node at (0,2.1) {C};
	\path[decorate, decoration={text along path, text={circunferencia}}][out=100](-2.14,0) to [in=139](.4,1.92);
\end{tikzpicture}
